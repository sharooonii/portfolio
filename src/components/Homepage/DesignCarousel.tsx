...
import { ImageItem, images } from "./design";
  ...
  <CardContainer className="py-2">
    <div className="space-y-1 cursor-pointer" onClick={onClick}>
      <img
        src={src}
        alt={alt}
        className="w-full rounded-lg h-auto hover:opacity-90 transition-opacity"
      />
    ...
    <div className="lightbox-container" onClick={onClose}>
      <div className="lightbox" onClick={(e) => e.stopPropagation()}>
        ...
        <Button
          variant="outline"
          size="icon"
          className={`-bottom-8 md:top-1/2 left-4 arrow-button ${
            isFirstImage ? "opacity-50 cursor-not-allowed" : "opacity-100"
          }`}
          onClick={(e) => {
            e.stopPropagation();
            onPrev();
        ...
        <Button
          variant="outline"
          size="icon"
          className={`-bottom-8 md:top-1/2 right-4 arrow-button ${
            isLastImage ? "opacity-50 cursor-not-allowed" : "opacity-100"
          }`}
          onClick={(e) => {
            e.stopPropagation();
            onNext();
...
export const DesignCarousel: FC = () => {
  ...
  return (
    <div
      id="design-carousel"
      className="container mx-auto space-y-4 my-20 px-4"
    >
      {isLargeScreen ? (
        ...
        <>
          ...
          <div className="grid grid-cols-6 gap-3">
            {images.slice(0, 5).map((img, index) => (
              <div key={index}>
                <PosterItem {...img} onClick={() => openLightbox(index)} />
              </div>
            ))}
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className={`arrow-button ${
                  isAtStart ? "arrow-disabled-state" : "arrow-active-state"
                }`}
                onClick={scrollPrev}
                disabled={isAtStart}
              >
              ...
              <Button
                ...
                size="icon"
                className={`arrow-button ${
                  isAtEnd ? "arrow-disabled-state" : "arrow-active-state"
                }`}
                onClick={scrollNext}
                disabled={isAtEnd}
              >
